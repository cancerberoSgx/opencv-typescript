 ...
 * @param resultingEdges Output vector to store the edges of the resulting MST.
 * @param algorithm Specifies which algorithm to use to compute the MST (see MSTAlgorithm).
 * @param root Starting node for the MST algorithm (only used for certain algorithms).
 */
export declare function buildMST(numNodes: number, inputEdges: any, resultingEdges: Vector<any>, algorithm: MSTAlgorithm, root?: number): boolean
 ...
 */
export declare function getClosestEllipsePoints(ellipse_params: any, points: InputArray, closest_pts: OutputArray): void
 ...
 *
 *
 * The Implementation is based on a paper by Aggarwal, Chang and Yap Aggarwal1985. They provide a  algorithm for finding the minimal convex polygon with k vertices enclosing a 2D convex polygon with n vertices (k < n). Since the minEnclosingConvexPolygon function takes a 2D point set as input, an additional preprocessing step of computing the convex hull of the 2D point set is required. The complexity of the convexHull function is  which is lower than . Thus the overall complexity of the function is .
 *
 * @param points Input vector of 2D points, stored in std::vector<> or Mat
 ...
 */
export declare function minEnclosingConvexPolygon(points: InputArray, polygon: OutputArray, k: number): number
