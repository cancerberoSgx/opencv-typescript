 ...
 *
 * @param signature1 First signature, a  floating-point matrix. Each row stores the point weight followed by the point coordinates. The matrix is allowed to have a single column (weights only) if the user-defined cost matrix is used. The weights must be non-negative and have at least one non-zero value.
 ...
 * @param cost User-defined  cost matrix. Also, if a cost matrix is used, lower boundary lowerBound cannot be calculated because it needs a metric function.
 ...
 * @param flow Resultant  flow matrix:  is a flow from  -th point of signature1 to  -th point of signature2 .
 */
export declare function EMD(signature1: InputArray, signature2: InputArray, distType: number, cost?: InputArray, lowerBound?: any, flow?: OutputArray): number
