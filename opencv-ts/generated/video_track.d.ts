...
import type { InputArray, InputOutputArray, OutputArray, OutputArrayOfArrays } from '../hacks/mat';
 ...
 */
export declare function buildOpticalFlowPyramid(img: InputArray, pyramid: OutputArrayOfArrays, winSize: Size, maxLevel: number, withDerivatives?: boolean, pyrBorder?: number, derivBorder?: number, tryReuseInputImage?: boolean): number
 ...
 */
export declare function calcOpticalFlowFarneback(prev: InputArray, next: InputArray, flow: InputOutputArray, pyr_scale: number, levels: number, winsize: number, iterations: number, poly_n: number, poly_sigma: number, flags: number): void
 ...
 *
 * Where:
 * - are the values of channel  at spatial location ,
 * - are the mean values of channel  over the masked region (if provided),
 * - is the number of channels (only 1 and 3 are currently supported),
 * - The sums run over all pixels  in the image domain (optionally restricted by mask).
 *
 *
 ...
 */
export declare function computeECC(templateImage: InputArray, inputImage: InputArray, inputMask?: InputArray): number
 ...
 * @param inputImage input image which should be warped with the final warpMatrix in order to provide an image similar to templateImage, same type as templateImage.
 * @param warpMatrix floating-point  or  mapping matrix (warp).
 * @param motionType parameter, specifying the type of motion: - **MOTION_TRANSLATION** sets a translational motion model; warpMatrix is  with the first  part being the unity matrix and the rest two parameters being estimated. - **MOTION_EUCLIDEAN** sets a Euclidean (rigid) transformation as motion model; three parameters are estimated; warpMatrix is . - **MOTION_AFFINE** sets an affine motion model (DEFAULT); six parameters are estimated; warpMatrix is . - **MOTION_HOMOGRAPHY** sets a homography as a motion model; eight parameters are estimated;`warpMatrix` is .
 ...
 */
export declare function findTransformECC(templateImage: InputArray, inputImage: InputArray, warpMatrix: InputOutputArray, motionType: number, criteria: TermCriteria, inputMask: InputArray, gaussFiltSize: number): number

/**
 * This is an overloaded member function, provided for convenience. It differs from the above function only in what argument(s) it accepts.
 */
export declare function findTransformECC(templateImage: InputArray, inputImage: InputArray, warpMatrix: InputOutputArray, motionType?: number, criteria?: TermCriteria, inputMask?: InputArray): number
 ...
 * @param sample sample image which should be warped with the final warpMatrix in order to provide an image similar to reference, same type as reference.
 * @param warpMatrix floating-point  or  mapping matrix (warp).
 ...
 */
export declare function findTransformECCMultiScale(reference: InputArray, sample: InputArray, warpMatrix: InputOutputArray, eccParams?: any, referenceMask?: InputArray, sampleMask?: InputArray): number
 ...
 * @param templateImage 1 or 3 channel template image; CV_8U, CV_16U, CV_32F, CV_64F type.
 * @param inputImage input image which should be warped with the final warpMatrix in order to provide an image similar to templateImage, same type as templateImage.
 * @param templateMask single-channel 8-bit mask for templateImage indicating valid pixels to be used in the alignment. Must have the same size as templateImage.
 * @param inputMask single-channel 8-bit mask for inputImage indicating valid pixels before warping. Must have the same size as inputImage.
 * @param warpMatrix floating-point  or  mapping matrix (warp).
 * @param motionType parameter, specifying the type of motion: - **MOTION_TRANSLATION** sets a translational motion model; warpMatrix is  with the first  part being the unity matrix and the rest two parameters being estimated. - **MOTION_EUCLIDEAN** sets a Euclidean (rigid) transformation as motion model; three parameters are estimated; warpMatrix is . - **MOTION_AFFINE** sets an affine motion model (DEFAULT); six parameters are estimated; warpMatrix is . - **MOTION_HOMOGRAPHY** sets a homography as a motion model; eight parameters are estimated; warpMatrix is .
 * @param criteria parameter, specifying the termination criteria of the ECC algorithm; criteria.epsilon defines the threshold of the increment in the correlation coefficient between two iterations (a negative criteria.epsilon makes criteria.maxcount the only termination criterion). Default values are shown in the declaration above.
 * @param gaussFiltSize size of the Gaussian blur filter used for smoothing images and masks before computing the alignment (DEFAULT: 5).
 */
export declare function findTransformECCWithMask(templateImage: InputArray, inputImage: InputArray, templateMask: InputArray, inputMask: InputArray, warpMatrix: InputOutputArray, motionType?: number, criteria?: TermCriteria, gaussFiltSize?: number): number
 ...
 */
export declare function meanShift(probImage: InputArray, window: any, criteria: TermCriteria): number
 ...
 */
export declare function writeOpticalFlow(path: any, flow: InputArray): boolean

export declare const OPTFLOW_USE_INITIAL_FLOW: number

export declare const OPTFLOW_LK_GET_MIN_EIGENVALS: number

export declare const OPTFLOW_FARNEBACK_GAUSSIAN: number

export declare const MOTION_TRANSLATION: number

export declare const MOTION_EUCLIDEAN: number

export declare const MOTION_AFFINE: number

export declare const MOTION_HOMOGRAPHY: number
